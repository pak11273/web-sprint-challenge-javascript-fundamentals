...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀: 
// The nestedFunction can access internal because of lexical scope.  Functions have access to their outer context (parent) all the way up to root.
...
const zooAnimals = [
  ...
  */
  ...
  function animalNames(animalNames){
    return animalNames.map(x => `name: ${x.animal_name}, scientific: ${x.scientific_name}`);
  }
  ...
  */
  ...
  function lowerCaseNames(names){
    return names.map(x => x.animal_name.toLowerCase());
  }
  ...
  */
  ...
  function lowPopulationAnimals(animals){
    return animals.filter(x => x.population < 5);
  }
  ...
  */
  ...
  function USApop(zoo){
    return zoo.reduce((acc, v) => acc + v.population, 0);
  }
...
let thing = new CubeMaker({length: 5, width: 5, height: 5});
